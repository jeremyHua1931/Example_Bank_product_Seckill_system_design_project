<template>
  <div class="forgetPwd">
    <div class="title">FORGET PWD</div>
    <input
      class="phone"
      type="text"
      placeholder=" PHONE"
...
import { find, getVerifyCode } from "@/api/fgtPwd/index.js";
...
export default {
  setup() {
    ...
    let submit = () => {
      ...
      data.isSubmit = false;
      find(data.phone, data.password, data.verifyCode).then((res) => {
        if (res.code == 200) {
          ElNotification({
            title: "修改成功",
            message: "",
    ...
    let getPhoneCode = () => {
      ...
      getVerifyCode(data.phone).then((res) => {
        if (res.code == 200) {
          ElNotification({
            title: "发送成功",
            message: "",
